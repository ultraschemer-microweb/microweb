<?php
  ...
  $client_secret = 'cb192d06-67c1-4b5e-9c17-7a7fbbd39e9d';
  $client_id = 'microweb';
  ...
  // Default values:
  $openid_server_address = 'https://<openid-server>'; 
  $redirect_uri_web = 'http://<openid-server?>/s/finish-consent-microweb.php';
  $redirect_uri_native = 'https://<openid-server?>/s/finish-consent-native-microweb.php';
  $redirect_uri_test = urlencode('/s/finish-consent.php'); /* Also can be: 'https://<openid-server?>/s/finish-consent.php'; */
  $server_backend_resource = $openid_server_address . '/microweb-backend';
  $realm = '<realm>';
  ...
  if(isset($_GET['state'])) {
    ...
    if(isset($state_info["r"])) {
      ...
      if($realm === '<another-realm>') {
        ...
        // and it's is saved here, since the client_secret and the client_id NEVER can
        // be accessible to the client!
        $client_secret = 'bbfc96bc-f6bc-4be9-8db5-09ff8eb2876';
      }
    }
  }
?>
